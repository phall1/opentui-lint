...
The strongest evidence a linter does not cry wolf is running every rule over
**real, working, expert-written OpenTUI code** and finding nothing wrong with
the code itself. A synthetic test cannot show that. This package runs the
check against two real sources, each pinned to an exact revision:
...
corpus code is wrong. This package's job is to tell you which, not just count
how many.
...
`{ ...recommended, ...four more rules }` (see
...
union, without paying for two lint passes over the same files. Every
...
- **`false-positive`** — the _rule_ is wrong. This is the whole reason this
  ...
  change this package cannot make itself (see [Files this package does not
  own](#files-this-package-does-not-own)).
...
where no theme was found and the rules had nothing to check: see the note in
`src/targets.ts` and the manual check described in the PR this package
shipped in.
...
19 occurrences (8 + 11) across both bindings, all following one shape. From
...
custom one, at `packages/solid/examples/session.tsx:141-144`:
...
That puts `components/ui/theme.ts` exactly where `opentui-lint`'s own
`CONVENTIONAL_UI_DIRS` discovery in
...
instead of the default path would prove nothing about the default path.
