...
describe("baseline matches live findings, exactly", () => {
  for (const target of targets) {
    test(target.id, async () => {
      ...
      expect(toolingErrors, `opentui-lint should never fail to parse real corpus source:\n${toolingErrors.join("\n")}`).toEqual(
        [],
      )

      const liveKeys = new Set(findings.map((f) => findingKey(target.id, f)))
      const baselineKeysForTarget = [...baseline.values()].filter((entry) => entry.target === target.id)
      ...
      const describeNew = newFindings
        .map((f) => `  [NEW, UNCLASSIFIED] ${f.ruleId} ${f.file}:${f.line}:${f.column}\n      ${f.message}`)
        .join("\n")
      const describeStale = staleEntries
        .map((e) => `  [STALE BASELINE] ${e.ruleId} ${e.file}:${e.line}:${e.column} (${e.classification})`)
        .join("\n")
...
test("every baseline entry names a target that still exists", () => {
  ...
  for (const entry of baseline.values()) {
    expect(targetIds.has(entry.target), `baseline.json references unknown target "${entry.target}"`).toBe(true)
  }
})
...
test("summary: findings by classification", async () => {
  ...
  for (const entry of baseline.values()) counts[entry.classification] = (counts[entry.classification] ?? 0) + 1
  ...
  expect(counts["true-positive"]! + counts["false-positive"]! + counts["acceptable"]!).toBe(baseline.size)
})
