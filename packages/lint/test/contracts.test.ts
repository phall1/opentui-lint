...
import { compileContracts, ContractConfigError, RESTYLE_CATEGORIES } from "../src/project/contracts.js"
...
describe("deny beats allow", () => {
  test("within one contract, a category in both allow and deny is denied", () => {
    const contracts = compileContracts([{ pattern: "^Badge$", allow: ["color", "spacing"], deny: ["color"] }])
...
describe("messages", () => {
  test("a denied verdict carries the matching contract's own message", () => {
    const contracts = compileContracts([{ pattern: "^Panel$", deny: ["border"], message: "Panel always has a border." }])
    expect(contracts.decide("Panel", "border")).toEqual({ allowed: false, message: "Panel always has a border." })
  })

  test("an allowed verdict carries no message", () => {
    const contracts = compileContracts([{ pattern: "^Panel$", allow: ["border"], message: "unused" }])
...
describe("config errors — never silently enforce less than was written", () => {
  ...
  test("an allow entry that names no real category throws", () => {
    expect(() => compileContracts([{ pattern: "^Button$", allow: ["colour"] }])).toThrow(ContractConfigError)
  })

  test("a deny entry that names no real category throws", () => {
    expect(() => compileContracts([{ pattern: "^Button$", deny: ["layout"] }])).toThrow(ContractConfigError)
  })
