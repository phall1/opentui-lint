...
function alphaToHex(raw: string | undefined): string {
  ...
  const text = raw.trim()
  const value = text.endsWith("%") ? Number.parseFloat(text.slice(0, -1)) / 100 : Number.parseFloat(text)
...
export function convertColor(value: string): ColorConversion | undefined {
  ...
  const rgb = /^rgba?\(\s*([^,\s/]+)[\s,]+([^,\s/]+)[\s,]+([^,\s/)]+)(?:\s*[,/]\s*([^)\s]+))?\s*\)$/i.exec(text)
  if (rgb) {
  ...
  const hsl = /^hsla?\(\s*([^,\s/]+)[\s,]+([^,\s/]+)[\s,]+([^,\s/)]+)(?:\s*[,/]\s*([^)\s]+))?\s*\)$/i.exec(text)
  if (hsl) {
