 ...
 */
function buildMessage(literalText: string, element: string, refs: string[], themeRel: string, typeDesc: string): string {
  if (refs.length > 1) {
    return (
...
export default defineRule(
  {
    type: "suggestion",
    docs: {
      description: "A literal that equals a theme token should be the token, so a theme switch reaches it.",
    ...
    schema: [
      {
        ...
        properties: {
          check: {
            ...
            description: "Token categories to check. Defaults to all three: density, borders, glyphs.",
          },
        },
  ...
  (context) => {
    ...
    function checkDensity(prop: string, valueNode: Node, reportNode: Node, element: string): void {
      ...
      context.report({ node: reportNode, message: buildMessage(`${prop}={${value}}`, element, refs, themeRel, "number") })
    }

    function checkBorders(prop: string, valueNode: Node, reportNode: Node, element: string): void {
      if (!enabled.has("borders") || prop !== "borderStyle" || borderStyleToken === undefined) return
      const value = staticString(valueNode)
      if (value === undefined || value !== borderStyleToken) return
      const refs = [`${accessPrefix()}.borders.style`]
      context.report({ node: reportNode, message: buildMessage(`${prop}="${value}"`, element, refs, themeRel, "string") })
    }
